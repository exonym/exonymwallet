...
public class SRSAGroupElementImpl implements HiddenOrderGroupElement {
   ...
   * @param val
   */
  public SRSAGroupElementImpl(final SRSAGroupImpl group, final BigInt val) {
    this.group = group;
    this.val = val;
  ...
  public HiddenOrderGroupElement op(final HiddenOrderGroupElement lhs) {
    if (lhs instanceof SRSAGroupElementImpl) {
      final SRSAGroupElementImpl lhsq = (SRSAGroupElementImpl) lhs;
      final BigInt result = val.multiply(lhsq.val).mod(modulus);
      //result = group.mapIntoGroup(result);
      return new SRSAGroupElementImpl(group, result);
    } else {
  ...
  public HiddenOrderGroupElement multOp(final BigInt exponent) {
    final BigInt result = val.modPow(exponent, modulus);
  ...
  @Override
  public SRSAGroupElementImpl invert() {
    final BigInt inv = val.modInverse(modulus);
    return new SRSAGroupElementImpl(group, inv);
  }
  ...
  public boolean equals(final Object obj) {
    ...
    final SRSAGroupElementImpl other = (SRSAGroupElementImpl) obj;
    if (group == null) {
