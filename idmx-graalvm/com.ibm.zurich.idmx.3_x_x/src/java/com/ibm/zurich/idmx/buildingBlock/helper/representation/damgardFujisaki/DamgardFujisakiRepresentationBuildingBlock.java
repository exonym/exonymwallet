...
/**
 * This block is used for generating a Damgard-Fujisaki (DF) commitment of a list of attributes
 * (including secrets), and proving knowledge of the representation. This block is intended to be
 * used as a sub-block by other building blocks. This block is very complex, as it has to satisfy
 * many use-cases.
 */
public class DamgardFujisakiRepresentationBuildingBlock
    extends RepresentationBuildingBlock<HiddenOrderGroup, HiddenOrderGroupElement> {

    @Inject
    public DamgardFujisakiRepresentationBuildingBlock(final RandomGeneration rg,
                                                    final BuildingBlockFactory bbFactory, final ExternalSecretsManager esManager, final Logger logger,
   ...
   * (1) It then generates the DF commitment C= PROD base^exponent (mod modulus) , possibly with the
   * help of the external device (though the External Secret Manager); (2) it proves knowledge of
   * the attribute values contained in the commitment possibly with the help of the external device
   * (through the External Secret Manager). It is the caller's responsibility to provide or set
   * attribute values that are available and require attribute values that must be copied from other
   * blocks.
   * 
   * @throws ConfigurationException
   */
  @Override
public ZkModuleProverCommitment<HiddenOrderGroupElement> getZkModuleProver(final SystemParameters systemParameters,
  final String identifierOfModule, final @Nullable URI identifierOfCredentialForSecret,
      final List<BaseForRepresentation> bases, final HiddenOrderGroup group,
      final @Nullable HiddenOrderGroupElement commitment, final @Nullable URI deviceUid, final String username, final @Nullable URI scope)
      throws ConfigurationException {
...
public ZkModuleVerifier getZkModuleVerifier(final SystemParameters systemParameters,
      ...
      final @Nullable HiddenOrderGroupElement commitment, final @Nullable String commitmentAsDValue,
      final @Nullable HiddenOrderGroup group) throws ProofException {
