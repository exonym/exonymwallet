...
public class VerifierModule extends ZkModuleImpl implements ZkModuleVerifier {
  ...
  private final RevocationAuthorityPublicKeyWrapper pkWrapper;

  public VerifierModule(final RevocationAuthorityPublicKeyBuildingBlock parent,
                        final String identifierOfModule, final SystemParameters systemParameters, final PublicKey publicKey) {

    super(parent, identifierOfModule);

    this.spWrapper = new EcryptSystemParametersWrapper(systemParameters);
    this.pkWrapper = new RevocationAuthorityPublicKeyWrapper(publicKey);
  }
  ...
  public boolean verify(final ZkVerifierStateVerify zkVerifier) throws ConfigurationException,
      ProofException {
    zkVerifier.checkNValue(getIdentifier() + ":raPk:c14n",
