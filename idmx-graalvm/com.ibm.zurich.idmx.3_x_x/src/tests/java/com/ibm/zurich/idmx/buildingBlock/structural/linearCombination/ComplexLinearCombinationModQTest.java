...
public class ComplexLinearCombinationModQTest {
...
//    sp = TestUtils.getResource("sp_default.xml", SystemParameters.class, this);
  ...
  public void testMultipleLhs() throws Exception {
    ...
    // Unlike the "Light" version, it's not a problem that A is computed via 2 different equations
    runTest(combinations, revealed, values);
  }
  ...
  public void testCircularDependency() throws Exception {
    ...
    // Unlike the "Light" version, the circular dependency is not a problem anymore
    // (assuming of course that the right attribute values are provided)
    runTest(combinations, revealed, values);
  }
  ...
  private void runTest(List<LinearCombination> combinations, List<String> revealed,
    ...
    for (LinearCombination combination : combinations) {
      ...
      ZkModuleProver zkp_lc =
          lc.getZkModuleProver(nameOfModule, combination.lhsAttribute, combination.constant,
              combination.terms, sp);
      zkp_l.add(zkp_lc);
      ZkModuleVerifier zkv_lc =
          lc.getZkModuleVerifier(nameOfModule, combination.lhsAttribute, combination.constant,
              combination.terms, sp);
      zkv_l.add(zkv_lc);
    }
